...
用法（在 backend/ 下執行）:
    python -m backend.llm.golden_set sample [--n 60]
    python -m backend.llm.golden_set agreement
    python -m backend.llm.golden_set merge
    python -m backend.llm.golden_set score <實驗報告.json 可多個>
    python -m backend.llm.golden_set --self-test
...
MODULE_DIR = Path(__file__).resolve().parent            # backend/llm/
# repo 根：llm → backend → <repo>。data/ 是共用語料的家。
