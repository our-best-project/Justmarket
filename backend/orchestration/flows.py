...
from backend.ingestion import APPROVED_CRAWLERS, crawl_and_ingest

# orchestration → backend → <repo>。子程序的工作目錄；套件本身已安裝進
...
def run_news_pipeline() -> None:
    ...
    commands = [
        ...
        [sys.executable, "-m", "backend.pipeline", "--stages", stages],
    ]
...
def update_market_index(dry_run: bool = False) -> None:
    from backend.market_index.daily_batch import run
...
def update_finmind(
    ...
    from backend.finmind.daily_batch import run
...
def rescore_market_validation() -> dict[str, int]:
    ...
    from backend.finmind.daily_batch import _db_url
