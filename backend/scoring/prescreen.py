...
用法：
    python -m backend.scoring.prescreen --dry-run     # 影子模式：只印決策不寫 DB
    python -m backend.scoring.prescreen --backtest    # 對已知星等事件回測召回率
"""
...
def prescreen_article(title: str | None, content: str | None) -> tuple[bool, str | None]:
    ...
    """
    from backend.clustering.dedup import periodic_type
...
def pre_llm_score(source_count: int, members: list[dict] | None,
    ...
    """
    from backend.scoring.importance import score_importance
...
def select_events_for_llm(conn, budget: int | None = None) -> list[str]:
    ...
    from backend.scoring.importance import fetch_heat_sources
