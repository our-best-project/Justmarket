...
```bash
uv run python -m backend.finmind.daily_batch --verify 2330:2026-07-03
```

```bash
uv run python -m backend.finmind.daily_batch --ticker 2330 --dry-run
```

```bash
uv run python -m backend finmind
```
...
```bash
uv run python -m backend.finmind.jobs
```
