...
- 收尾必跑健康記分板（pipeline_health）：空轉不能再裝成功。
  ...
  python -m backend.daily            # 跑整輪
  python -m backend.daily --list     # 只列步驟（給人與測試看）
"""
import argparse
import subprocess
import sys
...
STEPS: list[tuple[str, list[str]]] = [
    ("① 爬蟲（8 來源增量）",   ["-m", "backend.ingestion", "--all"]),
    ("② 向量化（pending→vectorized）", ["-m", "backend.embedding.bge_m3"]),
    ("③ 分群（vectorized→clustered）", ["-m", "backend.pipeline", "--stages", "embedding,clustering"]),
    ("④ 籌碼（FinMind 需求驅動＋假零仲裁）", ["-m", "backend.finmind.daily_batch", "--demand-only"]),
    ("④b 大盤指數（8 國日線）", ["-m", "backend.market_index.daily_batch"]),
    ("⑤ 篩選＋LLM＋評分",      ["-m", "backend.pipeline", "--stages", "llm,scoring"]),
    ("⑥ 健康記分板",           ["-m", "backend.pipeline_health"]),
]
