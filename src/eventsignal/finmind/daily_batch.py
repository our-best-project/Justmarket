...
用法：
  python -m eventsignal.finmind.daily_batch --verify 2330:2026-07-03  # 驗科目對應(單檔,免費可跑)
  python -m eventsignal.finmind.daily_batch --ticker 2330 --dry-run    # 單檔試跑不寫入
  python -m eventsignal.finmind.daily_batch                            # 正式：全量(撞額度等55分續抓)
  python -m eventsignal.finmind.daily_batch --demand-only              # 輕量：只跑需求集
"""
...
def run(tickers: list[str] | None = None, dry_run: bool = False,
    ...
    try:
        ...
        if not dry_run:
            try:
                from eventsignal.finmind.repair import arbitrate, scan_latest_day
