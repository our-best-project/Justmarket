...
給 Arku 的註冊範例（貼進 core/scheduler.py）：
    ...
    from eventsignal.finmind.jobs import daily_after_market_job
...
def rescore_pending_events(conn) -> tuple[int, int]:
    ...
    from eventsignal.scoring.market_validation import TICKER_4DIGIT, score_and_update_event

    # 下面用位置解包，強制 tuple_row——正式管線的連線是 dict_row（eventsignal.db.session），
    # 不指定的話解包出來會是欄位名稱字串
...
if __name__ == "__main__":
    # 手動觸發（測試用）：python -m eventsignal.finmind.jobs
