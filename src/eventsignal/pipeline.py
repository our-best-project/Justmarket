...
用法（在 backend/ 下）：
    python -m eventsignal.pipeline                    # DB 模式，沿用向量快取
    python -m eventsignal.pipeline --offline          # 離線檔案版
    python -m eventsignal.pipeline --llm-limit 5      # DB 模式但 LLM 只處理 5 個事件（試跑省額度）
    python -m eventsignal.pipeline --reembed          # 連向量化一起重跑（BGE-M3，CPU 約 40 分鐘）
"""
import argparse
import json
import os
...
from eventsignal.clustering import OUT_DIR as CLU_OUT
from eventsignal.clustering.dedup import assemble_events
from eventsignal.embedding.bge_m3 import (
    OUT_DIR as EMB_OUT,
)
from eventsignal.embedding.bge_m3 import (
    build_text,
...
def stage_embedding_db() -> tuple[list[dict], np.ndarray]:
    ...
    if not (os.path.exists(npy_path) and os.path.exists(meta_path)):
        raise SystemExit("找不到向量快取，請先跑：python -m eventsignal.embedding.bge_m3")
...
def stage_clustering(articles: list[dict], vectors: np.ndarray,
    ...
    from eventsignal.clustering.dedup import AGGLO_DISTANCE_THRESHOLD, DATE_GATE_DAYS
    from eventsignal.db.session import get_conn
...
def stage_llm(limit: int | None = None, prescreen: bool = True) -> None:
    ...
    """
    from eventsignal.llm.summarize import run_db_step

    selected: list[str] | None = None
    if prescreen:
        from eventsignal.db.session import get_conn
        from eventsignal.scoring.prescreen import expire_stale_events, select_events_for_llm
...
def main() -> None:
    ...
    if "embedding" in want and not args.no_prescreen:
        from eventsignal.db.session import get_conn
        from eventsignal.scoring.prescreen import apply_prescreen_pending
