...
```bash
uv run python -m eventsignal.llm.prompts
```

```bash
uv run python -m eventsignal.llm.summarize --self-test
```

上面兩個免金鑰、免網路，改完程式先跑它們。要打真 API：

```bash
uv run python -m eventsignal.llm.client
```

正式管線步驟（需 `DATABASE_URL`）：

```bash
uv run python -m eventsignal pipeline --stages llm
```
