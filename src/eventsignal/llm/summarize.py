...
用法:
    python -m eventsignal.llm.summarize             # 跑 DB 步驟（需 T03 表 + DATABASE_URL）
    python -m eventsignal.llm.summarize --limit 5   # 只處理前 5 個事件（試跑）
    python -m eventsignal.llm.summarize --self-test # 免金鑰/免 DB 自我測試
"""
...
def run_db_step(database_url: str | None = None, limit: int | None = None,
    ...
    from eventsignal.llm.client import _load_dotenv, create_client
...
if __name__ == "__main__":
    ...
    if args.self_test:
        ...
        from eventsignal.llm.samples import SAMPLE_EVENTS
