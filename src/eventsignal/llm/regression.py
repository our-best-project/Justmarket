...
用法:
    python -m eventsignal.llm.regression --self-test  # 離線驗證四種檢查器
    python -m eventsignal.llm.regression --dry        # 離線驗案例檔結構
    python -m eventsignal.llm.regression run          # 呼叫 LLM 跑完整回歸
"""
...
from eventsignal.llm.summarize import (
...
def run_regression(cases: list[dict]) -> int:
    ...
    from eventsignal.llm.client import create_client
