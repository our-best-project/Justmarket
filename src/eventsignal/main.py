...
啟動：`python -m eventsignal api`（等價於 uvicorn eventsignal.main:app）
...
from eventsignal.api import demo, events, market, tickers
from eventsignal.db.session import close_pool, open_pool
...
# 事件示意圖：檔案放 eventsignal/static/images/，不進資料庫。
...
def _serve() -> None:
    """`python -m eventsignal api` 的實作。

    等價於 `uvicorn eventsignal.main:app --host 0.0.0.0 --port 8000`。
